...
% ─────────────────────────────────────────────────────────────────────────────
\section{Nettoyage, normalisation et déduplication}
\label{sec:nettoyage}
% ─────────────────────────────────────────────────────────────────────────────

Cette section couvre les trois premières étapes du pipeline de préparation. Dans un premier temps, chaque source est nettoyée indépendamment selon ses anomalies propres (§\,\ref{subsec:nettoyage-saphelp} et §\,\ref{subsec:nettoyage-pdf}). Les fichiers sont ensuite fusionnés et soumis à une normalisation textuelle unifiée (§\,\ref{subsec:normalisation-commune}). Enfin, les doublons inter-sources et le bruit sémantique résiduel sont supprimés sur le corpus fusionné (§\,\ref{subsec:deduplication}).
% ── 2.1 ──────────────────────────────────────────────────────────────────────
...
\newcommand{\ptag}[2]{%
  ...
  \tikz[baseline=-0.55ex]{%
    \node[rectangle, rounded corners=2pt,
          fill=ptagcol!15, draw=ptagcol!55, line width=0.45pt,
          font=\tiny\bfseries\sffamily, text=ptagcol!85!black,
...
Les livres SAP en format PDF sont soumis à une extraction structurée préalable , générant des fichiers \texttt{structured\_content.json} pour chaque livre. Le fichier \texttt{clean\_scraped\_content.py} gère ensuite ces documents sans tenir compte du portail SAP Help.
...
Une fois nettoyées \emph{séparément}, les données des deux sources sont \textbf{fusionnées} dans un corpus unique, puis soumises à un pipeline de normalisation textuelle unifié implémenté dans \texttt{text\_utils.py}. Contrairement au nettoyage source-spécifique des sections précédentes : qui traitait les anomalies propres à chaque format (balises HTML pour le web, caractères CID pour les PDF) : cette étape applique des transformations \emph{identiques} à tous les documents, indépendamment de leur origine, afin de garantir un schéma de sortie homogène.
...
\hline
2 & Suppression bruit & Élimine les motifs de navigation (\textit{«~See also~»}, \textit{«~Rate this page~»}, etc.) & supprimés \\
\hline
3 & Segmentation lexicale & Redécoupe les mots concaténés issus de l’extraction web ou PDF & \texttt{contentadmin} → \texttt{content admin} \\
\hline
4 & Normalisation T-codes & Fusionne les codes transactionnels mal formatés & \texttt{"SM 37"} → \texttt{"SM37"} \\
\hline
5 & Normalisation Unicode & Applique NFC, supprime les caractères invisibles, met en minuscules & \texttt{(cid:34)} → espace \\
\hline
\end{tabularx}
\end{table}
...
\subsection{Suppression des doublons et du bruit}
\label{subsec:deduplication}

\subsubsection{Stratégie de déduplication en deux passes}
...
La figure~\ref{fig:dedup} illustre l'algorithme de déduplication en deux passes appliqué après la normalisation du corpus.
...
\begin{tikzpicture}[
    ...
    action/.style={rectangle, rounded corners=3pt, draw=SAPGray, thick,
                   ...
                   text centered, align=center, text width=3.4cm, font=\small},
...
La première passe, restreinte aux documents issus du
...
\subsubsection{Filtrage du bruit sémantique}
...
\begin{itemize}[leftmargin=*, label=\textbullet]
  ...
  \item \textbf{Extraction sensible à la structure et au contenu :}
  ...
  \begin{itemize}[leftmargin=2em, label=\textbullet]
    ...
    \item \textbf{Extraction basée sur la mise en page :}
    ...
    \item \textbf{Traitement enrichi par les métadonnées :}
  ...
  \item \textbf{Traitement multi-niveaux du contenu :}
  ...
  \begin{itemize}[leftmargin=2em, label=\textbullet]
    ...
    \item \textbf{Analyse au niveau des blocs :}
    ...
    \item \textbf{Traitement spécifique selon le type de contenu :}
...
\hline
Singles  & 73\,137 & Documents courts, un seul enregistrement, \textbf{vectorisé} \\
\hline
Parents  & 8\,795  & Documents longs, texte complet, stocké, \textbf{non vectorisé} \\
\hline
Enfants  & 32\,194 & Fragments de documents longs, \textbf{vectorisés} et indexés \\
\hline
\textbf{Total enregistrements} & \textbf{114\,126} & Tous types confondus \\
\hline
\textit{dont vectorisés (Singles + Enfants)} & \textit{105\,331} & \textit{Éligibles à l'embedding} \\
\hline
\end{tabular}
\end{table}

Les 8\,795 enregistrements de type \textit{Parent} sont conservés en base pour l'enrichissement de contexte lors de la récupération, mais ne portent pas de vecteur propre. Après application des filtres finaux de longueur et de qualité lors de l'ingestion dans PostgreSQL, \textbf{76\,463 vecteurs} sont effectivement indexés : ce chiffre constitue la taille opérationnelle de la base vectorielle utilisée dans toutes les évaluations ultérieures du rapport.
...
\hline
\textbf{bge-large-en-v1.5} & 1\,024 & Anglais & Très performant sur benchmarks BEIR & Monolingue \\
\hline
...
Ce chapitre a décrit l'ensemble du pipeline de préparation des données, depuis les sources brutes jusqu'à la base vectorielle opérationnelle. Les documents SAP ont été nettoyés, dédupliqués et normalisés à travers un processus rigoureux adapté à chaque source. Le découpage hiérarchique parent-enfant a permis de produire \textbf{76\,463 vecteurs} indexés, générés par le modèle \textbf{BAAI/bge-large-en-v1.5} et enrichis de métadonnées structurées. Stockés dans PostgreSQL avec pgvector, ces données sont indexées via les mécanismes HNSW et GIN, constituant ainsi le socle de la recherche hybride du système RAG. Le chapitre suivant présente la phase de modélisation, qui exploite cette base pour construire le pipeline de génération augmentée par récupération.
