...
L'objectif fondamental de l'étape de Modélisation (Modeling) dans la méthodologie CRISP-DM consiste à choisir et à mettre en œuvre différentes techniques de Machine Learning ou statistiques sur les données traitées, puis à optimiser leurs paramètres afin de déterminer le modèle qui convient le mieux aux objectifs du projet.% ─────────────────────────────────────────────────────────────
...
\section{Architecture globale du système RAG}
% ─────────────────────────────────────────────────────────────

\subsection{Vue d'ensemble}

Le système RAG conçu se divise en deux étapes majeures : une étape \textit{offline} pour l'ingestion et l'indexation, et une étape \textit{online} pour le traitement des requêtes. L'étape offline convertit les documents SAP bruts en une base vectorielle qui peut être interrogée. L'étape online gère la requête d'un utilisateur, en l'analysant sur le plan sémantique, en extrayant les passages les plus pertinents, et en produisant une réponse structurée à l'aide d'un LLM.

L'architecture repose sur les éléments clés suivants :
\begin{itemize}[leftmargin=2em]
  \item \textbf{Base de données vectorielle PostgreSQL (pgvector)}~: stocke les chunks de documentation SAP avec leurs embeddings, leurs métadonnées (module, T-code mentionné, type de source, fil d'Ariane) et un index de recherche plein texte (\texttt{tsvector}).
  \item \textbf{Modèle d'embedding BAAI/bge-large-en-v1.5}~: transforme les textes en vecteurs denses de dimension 1024, via la bibliothèque \texttt{SentenceTransformers}.
  \item \textbf{Cross-encoder de reranking}~: le modèle \texttt{cross-encoder/ms-marco-MiniLM-L-6-v2} évalue la pertinence fine de chaque chunk candidat par rapport à la requête.
  \item \textbf{LLM Ollama (llama3.2:3b)}~: modèle de génération de langage local, servi via l'API Ollama sur \texttt{http://localhost:11434}.
  \item \textbf{API FastAPI}~: couche REST exposant le pipeline à un frontend React/SAP Fiori.
\end{itemize}

\subsection{Flux de données}

Le flux global d'une requête utilisateur peut être résumé comme suit~: la question brute de l'utilisateur transite d'abord par une étape de compréhension sémantique (NLP), qui extrait l'intention, les entités SAP (T-codes, numéros de notes) et reformule la requête. La requête enrichie est ensuite vectorisée, puis soumise simultanément à une recherche vectorielle (similarité cosinus via pgvector) et à une recherche plein texte (BM25 via PostgreSQL \texttt{ts\_rank}). Les résultats des deux recherches sont fusionnés par \textit{Reciprocal Rank Fusion} (RRF), avant d'être reranqués par le cross-encoder. Les chunks retenus sont injectés dans un prompt structuré, puis transmis au LLM pour génération de la réponse finale. Un mécanisme de détection d'hallucinations valide la cohérence de la réponse avant son renvoi à l'utilisateur.
...
La reconnaissance des intentions s'appuie sur une liste structurée de règles regex désignée par \texttt{\_INTENT\_RULES}. Le système utilise les règles selon un ordre de priorité : le premier modèle trouvé détermine immédiatement l'intention finale. Les catégories principales sont :
\begin{itemize}
    \item \texttt{troubleshoot} : détection des erreurs et incidents techniques ;
    \item \texttt{how-to} : demandes de procédures ou d’actions ;
    \item \texttt{lookup} : requêtes de définition ou d’identification ;
    \item \texttt{explain} : catégorie par défaut lorsqu’aucune règle spécifique n’est détectée.
\end{itemize}

Par ailleurs, lorsqu’un T-code SAP est identifié dans la requête, l’intention est automatiquement forcée à \texttt{lookup}, car ces requêtes correspondent généralement à des demandes d’information ciblées.

L’identification du module SAP s’appuie sur deux mécanismes complémentaires :

\begin{itemize}
    \item le dictionnaire \texttt{\_MODULE\_KEYWORDS}, qui associe des expressions régulières à chaque module SAP ;
    
    \item le dictionnaire statique \texttt{\_TCODE\_TO\_MODULE}, permettant de mapper directement un T-code vers son module fonctionnel.
\end{itemize}

Le pipeline intègre également un mécanisme de résolution des pronoms anaphoriques via la fonction \texttt{\_resolve\_pronouns()}. Ce mécanisme améliore la continuité conversationnelle en réinjectant automatiquement le dernier T-code mentionné dans l’historique lorsque la requête contient un pronom.
...
sera automatiquement transformé en une requête contenant le T-code précédemment mentionné dans la conversation.

La fonction \texttt{enrich\_expanded\_query()} complète ensuite cette phase en enrichissant la requête avec des descriptions sémantiques prédéfinies pour chaque T-code connu, à l’aide du dictionnaire \texttt{\_TCODE\_EXPANSIONS}.
...
Le modèle \texttt{BAAI/bge-large-en-v1.5} convertit la requête enrichie en vecteur dense, étant chargé de manière paresseuse à l'aide de la fonction \texttt{get\_model()} dans un contexte de singleton. La bibliothèque \texttt{SentenceTransformers} garantit l'encodage avec normalisation L2. Conformément aux directives du modèle BGE pour la recherche asymétrique (requête courte vs passages longs), un préfixe distinctif \texttt{"Represent this sentence for searching relevant passages: "} est constamment rajouté en amont de la requête. La dimension du vecteur résultant est de 1024.

Une amélioration significative de la version V2 ([F8]) permet d'éviter le double calcul de l'embedding lorsque \texttt{clean\_query} et \texttt{expanded\_query} sont identiques : le vecteur est déterminé une seule fois et réutilisé.
...
La fonction \texttt{rrf\_fuse()} implémente l'algorithme RRF avec une pondération asymétrique favorable au BM25~:
...
La colonne \texttt{is\_parent} et \texttt{parent\_chunk\_id} révèlent l'implémentation d'une stratégie de chunking hiérarchique (\textit{Small-to-Big Retrieval}). Les chunks enfants (petits, précis) sont indexés pour la recherche, tandis que les chunks parents (plus larges, plus contextuels) peuvent être retournés au LLM pour une meilleure cohérence de contexte. Les requêtes de retrieval filtrent systématiquement les chunks parents (\texttt{is\_parent IS NOT TRUE}) afin de ne récupérer que des chunks feuilles lors de la recherche.
...
La fonction \texttt{check\_hallucinations()} met en œuvre un mécanisme de vérification post-génération visant à détecter d’éventuelles incohérences dans la réponse produite par le LLM. Elle compare les T-codes ainsi que les numéros de notes SAP mentionnés dans la réponse générée avec ceux effectivement présents dans les chunks de contexte récupérés lors de la phase de retrieval. Lorsqu’un T-code ou une note SAP apparaît dans la réponse sans être référencé dans le contexte fourni au modèle, cet élément est considéré comme une hallucination potentielle. Il est alors signalé dans le champ \texttt{warnings} de la réponse API, afin de permettre au frontend d’informer l’utilisateur si nécessaire.
% ───────────────────────────────────────────────────
...
\draw[arr] (cstore)  -- (end);
\end{tikzpicture}
\caption{Flowchart du pipeline RAG SAP V2 Fixed (rag\_pipeline\_v2\_fixed.py)}
