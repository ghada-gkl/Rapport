...
L'étape de Modélisation (Modeling) dans la méthodologie CRISP-DM consiste à choisir et à mettre en œuvre des techniques adaptées aux objectifs du projet. Dans un projet d'apprentissage supervisé classique, cette étape implique l'entraînement et l'optimisation d'un modèle sur des données annotées. \textbf{Dans le présent projet, aucun modèle n'est entraîné~: la « modélisation » désigne la conception et l'assemblage du pipeline RAG à partir de modèles pré-entraînés} (\textit{bge-large-en-v1.5}, \textit{llama3.2:3b}, cross-encoder) et de règles expertes (filtres BM25, seuils de reranking, stratégie de chunking hiérarchique). Le résultat de cette phase est donc une \textit{architecture de système} et non un modèle appris.% ─────────────────────────────────────────────────────────────
...
Le flux global d'une requête utilisateur peut être résumé comme suit : la question brute de l'utilisateur transite d'abord par une étape de compréhension sémantique (NLP), qui extrait l'intention, les entités SAP (T-codes, numéros de notes) et reformule la requête. La requête enrichie est ensuite vectorisée, puis soumise simultanément à une recherche vectorielle (similarité cosinus via pgvector) et à une recherche plein texte (BM25 via PostgreSQL \texttt{ts\_rank}). Les résultats des deux recherches sont fusionnés par \textbf{Reciprocal Rank Fusion (RRF)}, une technique qui combine les classements issus de sources hétérogènes en attribuant à chaque résultat un score inversement proportionnel à sa position dans chaque liste, favorisant ainsi les chunks bien classés dans plusieurs recherches simultanément. Ces résultats fusionnés sont ensuite reranqués par un \textbf{cross-encoder}, un modèle qui, contrairement aux embeddings calculés indépendamment, évalue la paire (requête, chunk) conjointement pour produire un score de pertinence plus fin et contextuellement précis. Les chunks retenus sont injectés dans un prompt structuré, puis transmis au LLM pour génération de la réponse finale. Un mécanisme de détection d'hallucinations valide la cohérence de la réponse avant son renvoi à l'utilisateur.
...
\subsection{Construction dynamique du prompt — \texttt{build\_prompt()}}
...
Avant la construction du prompt, chaque chunk subit une étape de \textbf{pré-synthèse sémantique} réalisée par la fonction \texttt{synthesize\_chunks()}. Cette étape extrait, par similarité cosinus avec le vecteur de la requête, uniquement les phrases les plus pertinentes du chunk~:
...
Le contrôle anti-hallucination opère en deux étapes complémentaires après la génération~:

\textbf{Étape 1 — Détection par \texttt{check\_hallucinations()}.} La fonction compare les T-codes et numéros de notes SAP présents dans la réponse générée avec ceux effectivement contenus dans les chunks récupérés. Tout T-code connu (\texttt{KNOWN\_TCODES}) mentionné dans la réponse sans apparaître dans le contexte est signalé comme hallucination potentielle~:

\[
\text{T-codes halluc.} = \bigl(\text{T-codes}(\text{réponse}) \setminus \text{T-codes}(\text{contexte})\bigr) \cap \texttt{KNOWN\_TCODES}
\]

\textbf{Étape 2 — Sanitisation par \texttt{sanitize\_answer()}.} Les phrases de la réponse dont \textit{tous} les T-codes mentionnés sont hallucinés (non présents dans le contexte) sont supprimées de la réponse finale avant renvoi à l’utilisateur. Les phrases mixtes (T-codes halluc. + T-codes légitimes) sont conservées et signalées en \texttt{warnings} dans la réponse API.

\begin{definitionbox}{Choix de conception — Contrôle déterministe vs RAGAS}
La détection d’hallucinations par correspondance T-code est délibérément déterministe, contrairement aux métriques LLM-as-Judge (Faithfulness RAGAS). Ce choix garantit une vérification instantanée ($<$1 ms), sans appel LLM supplémentaire, avec un comportement prévisible et explicable. Les deux approches sont complémentaires~: le contrôle déterministe attrape les hallucinations d’entités SAP précises~; RAGAS évalue la fidélité sémantique globale.
\end{definitionbox}
