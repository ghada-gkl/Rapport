...
\begin{tcolorbox}[
  ...
  title={Limite du modèle-juge \texttt{llama3.2:3b} — métriques JSON-dépendantes},
  attach boxed title to top left={yshift=-2mm, xshift=6mm},
  boxed title style={colback=SAPOrange, rounded corners},
  rounded corners, arc=3mm,
  left=8pt, right=8pt, top=5pt, bottom=5pt
]
Les métriques \textbf{Faithfulness} et \textbf{Context Precision} nécessitent que le modèle-juge génère
une sortie JSON strictement structurée. Le modèle \texttt{llama3.2:3b} (3\,Mds de paramètres) ne
respecte pas ce schéma de manière fiable~: \textbf{100\,\% des appels échouent} avec une erreur de
\textit{parsing}. Un modèle plus puissant (GPT-4, Claude Sonnet, ou Ollama $\geq$7\,B) est requis pour
calculer ces métriques de manière fiable.
\end{tcolorbox}
