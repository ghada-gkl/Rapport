...
Faithfulness
  ...
  & \textcolor{SAPGray}{Non calculable~: \texttt{llama3.2:3b} ne produit pas la sortie JSON structurée requise par RAGAS (100\,\% d'échecs de parsing). Un juge LLM plus puissant est nécessaire (voir §~\ref{subsec:limites-ragas})} \\
\rowcolor{SAPLightGray}
Context Precision
  & \textcolor{SAPGray}{\bfseries N/A}
  & \textcolor{SAPRed}{\bfseries Non}
  & \textcolor{SAPGray}{Non calculable pour la même raison~: dépendance au format JSON strict du juge LLM (voir §~\ref{subsec:limites-ragas})} \\
\bottomrule
\end{tabularx}
\end{table}
