...
Les données de surveillance opérationnelle sont extraites automatiquement des systèmes SAP de production par la couche PyRFC, le connecteur Python pour les Remote Function Calls SAP. Cela nous permet d'appeler directement les modules de fonction SAP, rendant l'extraction répétable, planifiable et traçable.

Chaque extraction produit des enregistrements des serveurs des clients, et chaque TCode a une structure de données bien définie. Voici quelques exemples de transactions couvertes : journaux d'erreurs ABAP (ST22), statuts de jobs (SM37), espace de base de données (DB02), entrées de verrouillage (SM12), processus de travail (SM50, SM66), files qRFC (SMQ1, SMQ2), messagerie (SOST), ainsi que les métriques système comme le CPU, la mémoire et le système de fichiers.
...
La base de connaissances de l'assistant IA (RAG) est construite à partir de deux sous-sources documentaires principales.

La première est constituée de \textbf{297 supports de cours SAP au format PDF} : administration Basis (ADM100, ADM505), développement ABAP (BC100, BC400), modules fonctionnels MM, SD, FI, ainsi que des formations spécialisées sur NetWeaver, S/4HANA, Fiori et la sécurité. Ces documents proviennent du programme de formation accrédité SAP et constituent la référence pédagogique la plus complète pour les techniciens SAP.

La seconde source est le \textbf{portail d'aide en ligne SAP} (\texttt{help.sap.com}), dont plusieurs sections thématiques ont été extraites par web scraping, par exemple : \texttt{https://help.sap.com/crawler/PRODUCTION/SAP\_NETWEAVER\_740/7.4.27/en-US}. Ce portail fournit une documentation orientée configuration, codes d'erreur, paramètres système et notes de correction (SAP Notes), complémentaire aux supports de formation des systèmes SAP.\paragraph{Extraction PDF par agents parallèles.}
