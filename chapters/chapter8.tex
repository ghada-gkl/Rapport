...
Le système déployé repose sur une architecture à trois couches distinctes communiquant via des interfaces bien définies. La figure~\ref{fig:deploy_arch} illustre l'ensemble des composants et leurs interconnexions.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{ChatPanel : Tiroir latéral}~: exporté depuis \texttt{Chatbot.jsx}, ce panneau coulissant est intégré dans le composant \texttt{AppShell.jsx} et rendu accessible depuis toutes les pages de l'application via le bouton IA dans la \texttt{ShellBar}.
\end{itemize}


\subsection{Intelligence de navigation : T-codes vers pages monitoring}

Une fonctionnalité avancée du chatbot est sa capacité à détecter dans le message de l'utilisateur une \textbf{intention de navigation} vers une page de monitoring, et à y rediriger automatiquement. 
...
\subsection{Interface du chatbot pour Admin}

Les figures suivantes présentent l'interface du chatbot  illustrant la réponse générée avec ses sources ainsi que le mécanisme de retour administrateur.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{\detokenize{images/Screenshot 2026-05-29 164204.png}}
\caption[Chatbot SAP : réponse avec sources]{Réponse du chatbot assistant SAP avec liste des sources documentaires et widget de feedback}
\label{fig:chat_response}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{\detokenize{images/Screenshot 2026-05-29 164230.png}}
\caption[Chatbot SAP : mécanisme de correction]{Mécanisme de correction activé après sélection de «~À améliorer~» par l'utilisateur}
\label{fig:chat_feedback}
\end{figure}

% ─────────────────────────────────────────────────────────────
\section{Benchmark des configurations de pipeline}
% ─────────────────────────────────────────────────────────────
...
\begin{itemize}[leftmargin=2em]
  \item Plus le système consulte de documents, meilleures sont ses réponses. Les deux modèles donnent de bons résultats lorsqu'on leur fournit suffisamment de sources.

  \item Les réponses les plus pertinentes apparaissent toujours en premier, ce qui facilite la lecture et l'exploitation des résultats.

  \item Les deux modèles se comportent différemment selon leur configuration, ce qui influence la précision des réponses produites.

  \item LLaMA 3.2:3b est plus rapide, tandis que Qwen 2.5:7b fournit des réponses plus complètes. Il est possible de choisir l'un ou l'autre selon les besoins de l'utilisateur.
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Mise à jour incrémentale de la base de connaissances}~: Un pipeline de synchronisation incrémentale (déjà prototypé en interne) permettrait d'enrichir automatiquement la base documentaire à mesure que de nouvelles notes SAP ou documentations sont publiées.
