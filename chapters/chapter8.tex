...
La phase de Déploiement représente l’étape finale de la méthodologie CRISP-DM. Elle a pour objectif de mettre en exploitation le modèle conçu durant la phase de Modélisation, en l’intégrant dans un environnement de production destiné aux utilisateurs finaux. 
% ─────────────────────────────────────────────────────────────
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rectangle, rounded corners=5pt,
              ...
              text centered, align=center, font=\footnotesize, fill=white},
  arr/.style={-{Stealth[length=6pt]}, thick},
  lbl/.style={font=\scriptsize, fill=white, inner sep=2pt, rounded corners=2pt},
]

% ── Boîtes Couche 1 : Frontend (y=8.0) ──────────────────────
\node[box, draw=SAPBlue] (shell)    at (-5.0, 8.0) {\textbf{ShellBar}\\\scriptsize Bouton IA};
\node[box, draw=SAPBlue] (panel)    at (-1.5, 8.0) {\textbf{ChatPanel}\\\scriptsize Tiroir latéral};
\node[box, draw=SAPBlue] (chatpage) at ( 2.0, 8.0) {\textbf{Chatbot.jsx}\\\scriptsize Page dédiée};
\node[box, draw=SAPBlue] (pages)    at ( 5.5, 8.0) {\textbf{25+ Pages}\\\scriptsize Monitoring};

% ── Boîtes Couche 2 : CAP (y=5.0) ───────────────────────────
\node[box, draw=SAPGreen] (chatSrv) at (-3.5, 5.0) {\textbf{chat-service}\\\scriptsize OData Action};
\node[box, draw=SAPGreen] (monSrv)  at ( 0.5, 5.0) {\textbf{monitoring-service}\\\scriptsize 18 entités OData};
\node[box, draw=SAPGreen] (mongo)   at ( 4.5, 5.0) {\textbf{MongoDB Atlas}\\\scriptsize Données monitoring};

% ── Boîtes Couche 3 : RAG (y=2.0) ───────────────────────────
\node[box, draw=SAPOrange] (api)    at (-5.0, 2.0) {\textbf{api.py}\\\scriptsize FastAPI};
\node[box, draw=SAPOrange] (pipe)   at (-1.5, 2.0) {\textbf{rag\_pipelinev2}\\\scriptsize Pipeline RAG};
\node[box, draw=SAPOrange] (ollama) at ( 2.0, 2.0) {\textbf{Ollama}\\\scriptsize llama3.2:3b (11434)};
\node[box, draw=SAPOrange] (pg)     at ( 5.5, 2.0) {\textbf{PostgreSQL}\\\scriptsize pgvector (5432)};

% ── Fonds des couches (fit autour des boîtes) ────────────────
\begin{scope}[on background layer]
  \node[draw=SAPBlue,   fill=SAPLightBlue,   rounded corners=10pt, inner sep=12pt,
        fit=(shell)(panel)(chatpage)(pages)]   (layer1) {};
  \node[draw=SAPGreen,  fill=SAPLightGreen,  rounded corners=10pt, inner sep=12pt,
        fit=(chatSrv)(monSrv)(mongo)]          (layer2) {};
  \node[draw=SAPOrange, fill=SAPLightOrange, rounded corners=10pt, inner sep=12pt,
        fit=(api)(pipe)(ollama)(pg)]           (layer3) {};
\end{scope}

% ── Labels des couches (intérieur haut-gauche) ───────────────
\node[anchor=north west, font=\footnotesize\bfseries\color{SAPBlue}]
  at ([xshift=7pt, yshift=-6pt]layer1.north west)
  {Couche Présentation — Frontend React / SAP UI5 (port 3000)};
\node[anchor=north west, font=\footnotesize\bfseries\color{SAPGreen}]
  at ([xshift=7pt, yshift=-6pt]layer2.north west)
  {Couche Métier — SAP CAP / Express (port 4004)};
\node[anchor=north west, font=\footnotesize\bfseries\color{SAPOrange}]
  at ([xshift=7pt, yshift=-6pt]layer3.north west)
  {Couche RAG — Python / FastAPI (port 8000)};

% ── Flèches inter-couches ────────────────────────────────────
% ChatPanel → chat-service (flèche principale + label)
\draw[arr, SAPBlue!80]
  (panel.south)
  -- node[lbl, right]{\texttt{POST /odata/v4/chat/query}} ++(0,-1.0)
  -| (chatSrv.north);

% Chatbot.jsx → chat-service (chemin secondaire)
\draw[arr, SAPBlue!50, dashed]
  (chatpage.south) -- ++(0,-1.4) -| (chatSrv.north);

% chat-service → api.py (appel RAG + label)
\draw[arr, SAPGreen!80]
  (chatSrv.south)
  -- node[lbl, right]{\texttt{POST http://localhost:8000/query}} ++(0,-1.0)
  -| (api.north);

% Flèches internes couche RAG
\draw[arr, SAPOrange] (api.east)    -- (pipe.west);
\draw[arr, SAPOrange] (pipe.east)   -- (ollama.west);
% pipe → PostgreSQL : routage sous la ligne pour éviter de traverser Ollama
\draw[arr, SAPOrange]
  (pipe.south) -- ++(0,-0.45)
  -- ($(pg.south) + (0,-0.45)$)
  -- (pg.south);

% Flèche interne couche CAP
\draw[arr, SAPGreen] (monSrv.east) -- (mongo.west);

\end{tikzpicture}%
}
\caption{Architecture globale de déploiement du système RAG intégré dans AutoMoni}
...
La base de données PostgreSQL hébergée à l'adresse \texttt{45.159.228.43:5432} contient la table \texttt{sap\_chunks} avec \textbf{76~463 vecteurs} indexés. L'index HNSW (\textit{Hierarchical Navigable Small World}) a été créé avec les paramètres suivants~:
...
Cet index permet une recherche approximative du plus proche voisin en temps sous-linéaire, avec un temps de réponse moyen inférieur à 100~ms pour une collection de 76~463 vecteurs de dimension 1024.
...
\hline
\texttt{NUM\_CTX} & 8~192 & Fenêtre de contexte du LLM Ollama (en tokens) \\
\hline
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Page dédiée} (\texttt{/chatbot})~: le composant \texttt{Chatbot.jsx} (723 lignes) offre une interface complète avec historique de conversations organisées en sessions, liste des conversations précédentes, zone de réponse avec rendu Markdown via \texttt{react-markdown}, et boutons d'action (nouvelle conversation, effacement).
...
\caption{Interface de la page chatbot dédiée avec historique de conversations}
...
\caption{ChatPanel intégré en tiroir latéral sur une page de monitoring SAP}
\label{fig:chatpanel}
\end{figure}

% ─────────────────────────────────────────────────────────────
\section{Évaluation et benchmarking}
% ─────────────────────────────────────────────────────────────
...
\caption{Résultats du benchmark RAG — LLaMA 3.2:3b vs Qwen 2.5:7b (12 mai 2026, 76~463 vecteurs)}
\label{fig:benchmark}
\end{figure}
